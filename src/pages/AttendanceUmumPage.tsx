...
export default function AttendanceUmumPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!event || !event.spreadsheet_url_umum) {
    ...
    try {
      ...
      if (submitted) {
        setSuccess(true);
        setFormData({ nama: '', prodi: '', nomor_hp: '' });
      } else {
      ...
      <div className="max-w-2xl mx-auto">
        <GlassCard className="p-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            />
            <div>
              <h1 className="text-2xl font-bold text-white">Absensi Peserta Umum</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-white/40 focus:border-purple-400 focus:outline-none"
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-4 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl font-semibold hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed"
            >
